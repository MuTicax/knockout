...
local function KnockedOutLoop(ped)
    ...
    lib.showTextUI('Vous êtes actuellement assomé ...', {position = "top-center"})
    ...
    if event == "CEventNetworkEntityDamage" then
        ...
        if victim == PlayerId() then
            ...
            if IsPedInMeleeCombat(ped) then
                ...
                elseif HasPedBeenDamagedByWeapon(ped, GetHashKey("WEAPON_NIGHTSTICK"), 0) then
                    if GetEntityHealth(ped) < Config.Health then
                        if not knockedOut then
                            knockedOut = true
                            SetEntityInvincible(ped, true)
                ...
                elseif HasPedBeenDamagedByWeapon(ped, GetHashKey("WEAPON_BAT"), 0) then
                    if GetEntityHealth(ped) < Config.Health then
                        if not knockedOut then
                            knockedOut = true
                            SetEntityInvincible(ped, true)
                ...
                elseif HasPedBeenDamagedByWeapon(ped, GetHashKey("WEAPON_KNUCKLE"), 0) then
                    if GetEntityHealth(ped) < Config.Health then
                        if not knockedOut then
                            knockedOut = true
                            SetEntityInvincible(ped, true)
